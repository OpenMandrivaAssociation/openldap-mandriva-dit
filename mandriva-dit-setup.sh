...
if [ -z "$mypass" ]; then
	...
	echo "Please choose a password for this account:"
        myslapdpass=`make_temp`
	while /bin/true; do
		$SLAPPASSWD -h {SSHA} | tee $myslapdpass
		pass=`grep SSHA $myslapdpass`
		if [ -n "$pass" ]; then
			break
		fi
	done
...
# Check/fix schema locations
awk '/^include\s*.*.schema/ {print $2}' $myslapdconf | while read schema
do
	if [ ! -e $schema ]
	then
		newschema=''
		schemaname=${schema##*/}
		for schemadir in /etc/openldap/schema /usr/share/openldap2.4/schema /etc/openldap2.4/schema
		do
			if [ -e $schemadir/$schemaname ]
			then
				newschema=$schemadir/$schemaname
				perl -pi -e "s,^(include\s*)$schema,\${1}$newschema,g" $myslapdconf
				break
			fi
		done
		if [ "$newschema" == "" ]
		then
			echo "Couldn't find location of required schema $schema" >&2
			exit 1
		fi
	fi
done
# check modules to load
modulepath=$(awk '/^modulepath/ {print $2}' $myslapdconf)
if [ -z "$modulepath" ]
then echo "Module path is empty, we hope all overlays are built-in"
fi
awk '/^moduleload/ {print $2}' $myslapdconf |while read module
do
	if [ ! -e $modulepath/$module ]
	then
		echo "$modulepath/$module does not exist, removing moduleload statement" >&2
		perl -pi -e "s,^moduleload\s*$module,,g" $myslapdconf
	fi
done
# check pid file location
for type in pid args
do
	file=$(awk "/^${type}file/ {print \$2}" $myslapdconf)
	dir=$(dirname $file)
	if [ ! -d $dir ]
	then
		if [ -d "/var/run/openldap" ]
		then
			perl -pi -e "s,(^${type}file\s*)$dir,\${1}/var/run/openldap,g" $myslapdconf
		fi
	fi
done
# check cert location
awk '/^TLS.*File/ {print $2}' $myslapdconf |while read cert
do
	if [ ! -e $cert ]
	then
		if [ -e "/etc/pki/tls/certs/slapd.pem" ]
		then
			perl -pi -e "s,(^TLS.*File\s*)$cert,\$1/etc/pki/tls/certs/slapd.pem,g" $myslapdconf
		fi
	fi
done
